    ...
    for (auto function : functions)
    {
        for (uint8_t i = 0; i < TOTAL; i++)
        {
            ...
            case UNCHECKED_CALL: {
                ...
                bool rootException = rootCallResponse.payload.inst == Instruction::INVALID &&
                                     !rootCallResponse.level;
                for (auto ctx : function)
                {
                    if ((!rootException && ctx.payload.inst == Instruction::INVALID && ctx.level) ||
                        !ctx.payload.isChecked)
                    // if (!rootException && ctx.payload.inst == Instruction::INVALID && ctx.level )
                    {
            ...
            case TXORIGIN: {
                ...
                for (auto ctx : function)
                {
                    // has_transfer = has_transfer || ctx.payload.wei > 0;
                    //                                           only process payable function
                    if (ctx.payload.inst == Instruction::ORIGIN)
                    {
                        has_TxOrigin = has_TxOrigin || true;
                        lastpc = ctx.payload.pc;
                        // reached_jumpi = false;
                    }
                    if (has_TxOrigin && !reached_jumpi)
                        if (ctx.payload.inst == Instruction::ADDRESS)
                        {
                            // has_sha3 = has_sha3 || true;
                            distinctions[i].insert((uint16_t)lastpc);
                        }
                    reached_jumpi = reached_jumpi || ctx.payload.inst == Instruction::JUMPCI;
                    if (ctx.payload.inst == Instruction::EQ && ctx.payload.pc - lastpc < 3)
                    {
                        lastpc = ctx.payload.pc;
                    }
                    if (ctx.payload.inst == Instruction::JUMPCI && ctx.payload.pc - lastpc < 9)
                ...
                if (has_TxOrigin && in_condition)
                    vulnerabilities[i]++;
                break;
            }
            case FALSEASSERT: {
                bool has_AssertFalse = false;
                // bool has_sha3 = false;
                bool reached_jumpi = false;
                bool in_condition = false;
                u256 lastpc = 0;
                for (auto ctx : function)
                {
                    // has_transfer = has_transfer || ctx.payload.wei > 0;
                    //                                           only process payable function
                    if (ctx.payload.inst == Instruction::INVALID)
                    {
                        has_AssertFalse = has_AssertFalse || true;
                        lastpc = ctx.payload.pc;
                        // reached_jumpi = false;
                    }
                    if (has_AssertFalse && !reached_jumpi)
                        if (ctx.payload.inst == Instruction::JUMPI)
                        {
                            // has_sha3 = has_sha3 || true;
                            distinctions[i].insert((uint16_t)lastpc);
                        }
                ...
                if (has_AssertFalse && in_condition)
                    vulnerabilities[i]++;
                break;
            }
            case FALSESUICIDE: { 
                bool has_SuicideFalse = false;
                // bool has_sha3 = false;
                bool reached_jumpi = false;
                u256 lastpc = 0;
                for (auto ctx : function)
                {
                    // has_transfer = has_transfer || ctx.payload.wei > 0;
                    //                                           only process payable function
                    if (ctx.payload.inst == Instruction::SUICIDE)
                    {
                        has_SuicideFalse = has_SuicideFalse || true;
                        lastpc = ctx.payload.pc;
                        // reached_jumpi = false;
                    }
                    if (ctx.payload.inst == Instruction::AND && ctx.payload.pc - lastpc < 3)
                    {
                        lastpc = ctx.payload.pc;
                        distinctions[i].insert((uint16_t)lastpc);
                    }
