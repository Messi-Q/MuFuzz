    ...
    for (auto progIt : progInfo)
    {
        ...
        for (uint64_t i = 0; i < decompressedSourcemap.size() && i < opcodes.size(); i++)
        {
            if (!is_prefuzz)
            {
                ...
                /*find valid Tx.origin */
                if(get<1>(opcodes[i]) == Instruction::ORIGIN)
                {
                    auto offset = decompressedSourcemap[i][0];
                    auto len = decompressedSourcemap[i][1];
                    auto snippet = contractInfo.source.substr(offset, len);
                     // cout << "DELEGATECALL" << endl;
                    if (snippet.find("tx.origin") != string::npos) 
                    {
                        Logger::info("------------TransactionOrigin snippet---------");
                        Logger::info(snippet);
                        Logger::info(to_string(get<0>(opcodes[i])));
                        Logger::info("------------TransactionOrigin snippet end---------");
                        if (isRuntime)
                            runtimeTxOrigin.insert(get<0>(opcodes[i]));
                        else
                            deploymentTxOrigin.insert(get<0>(opcodes[i]));
                    }
                }
                /*find validAssert */
                if(get<1>(opcodes[i]) == Instruction::INVALID)
                {
                    auto offset = decompressedSourcemap[i][0];
                    auto len = decompressedSourcemap[i][1];
                    auto snippet = contractInfo.source.substr(offset, len);
                     // cout << "DELEGATECALL" << endl;
                    if (snippet.find("assert") != string::npos)
                    {
                        // Logger::info("------------TransactionOrigin snippet---------");
                        // Logger::info(snippet);
                        // Logger::info(to_string(get<0>(opcodes[i])));
                        // Logger::info("------------TransactionOrigin snippet end---------");
                        if (isRuntime)
                            runtimeAssert.insert(get<0>(opcodes[i]));
                        else
                            deploymentAssert.insert(get<0>(opcodes[i]));
                    }
                }
                /* find valid Suicide */
                if(get<1>(opcodes[i]) == Instruction::SUICIDE)
                {
                    auto offset = decompressedSourcemap[i][0];
                    auto len = decompressedSourcemap[i][1];
                    auto snippet = contractInfo.source.substr(offset, len);
                     // cout << "DELEGATECALL" << endl;
                    if (snippet.find("suicide") != string::npos || snippet.find("selfdestruct") != string::npos)
                    {
                        // Logger::info("------------TransactionOrigin snippet---------");
                        // Logger::info(snippet);
                        // Logger::info(to_string(get<0>(opcodes[i])));
                        // Logger::info("------------TransactionOrigin snippet end---------");
                        if (isRuntime)
                            runtimeSuicide.insert(get<0>(opcodes[i]));
                        else
                            deploymentSuicide.insert(get<0>(opcodes[i]));
                    }
                }
            }
...
pair<unordered_set<uint64_t>, unordered_set<uint64_t>> BytecodeBranch::findValidTxOrigin()
{
    return make_pair(deploymentTxOrigin,runtimeTxOrigin);
}
pair<unordered_set<uint64_t>, unordered_set<uint64_t>> BytecodeBranch::findValidAssert()
{
    return make_pair(deploymentAssert,runtimeAssert);
}
pair<unordered_set<uint64_t>, unordered_set<uint64_t>> BytecodeBranch::findValidSuicide()
{
    return make_pair(deploymentSuicide,runtimeSuicide);
}
