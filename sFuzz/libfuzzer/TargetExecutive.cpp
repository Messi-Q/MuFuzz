            ...
            case Instruction::STATICCALL: {  //                     call other function,
                ...
                if (inst == Instruction::CALL || inst == Instruction::DELEGATECALL)
                {
                    ...
                    auto isExit = get<0>(valids).count(pc)  || get<1>(valids).count(pc) ||
                                  ...
                                  get<10>(valids).count(pc) || get<11>(valids).count(pc)||
                                  get<12>(valids).count(pc) || get<13>(valids).count(pc)||
                                  get<14>(valids).count(pc) || get<15>(valids).count(pc);
                    if (isExit){
                ...
                if (inst == Instruction::SUICIDE || inst == Instruction::NUMBER ||
                    ...
                    inst == Instruction::MUL || inst == Instruction::SHA3 || 
                    inst == Instruction::BALANCE || inst == Instruction::ORIGIN )
                {
                    ...
                    if (inst == Instruction::ORIGIN){
                        auto push1 = vm->stack()[stackSize - 1];
                        if(push1 == Instruction::AND && inst == Instruction::ORIGIN){
                            auto recordable = recordParam.isDeployment && get<10>(valids).count(pc);
                            recordable =
                                recordable || !recordParam.isDeployment && get<11>(valids).count(pc);
                            if (!recordable)
                                break;
                           }
                        }
                    // vector<u256>::size_type stackSize = vm->stack().size();
                        if (inst == Instruction::INVALID){
                            bool flag = 0;
                            for(int i = 7;i>0;i--){
                                auto push1 = vm->stack()[stackSize - i];
                                if(push1 == Instruction::AND || push1 == Instruction::EQ)
                                {
                                    flag =1;
                                    break;
                                }
                            ...
                            if(flag == 1){
                                auto recordable = recordParam.isDeployment && get<12>(valids).count(pc);
                                recordable =
                                    recordable || !recordParam.isDeployment && get<13>(valids).count(pc);
                                if (!recordable)
                                    break;
                                }
                            }
                    ...
                    if (inst == Instruction::SUICIDE){
                        ...
                        // recordable =
                        //     recordable || !recordParam.isDeployment && get<7>(valids).count(pc);
                        auto recordable = recordParam.isDeployment && get<14>(valids).count(pc);
                        recordable =
                            recordable || !recordParam.isDeployment && get<15>(valids).count(pc);
                        if (!recordable)
                            break;
                    }
...
//                     inst == Instruction::TIMESTAMP || inst == Instruction::INVALID ||
//                     inst == Instruction::ADD || inst == Instruction::SUB ||
//                     inst == Instruction::SHA3 || inst == Instruction::BALANCE || inst == Instruction::ORIGIN)
//                 {
//                     if (inst == Instruction::TIMESTAMP)
//                     {
//                         // auto recordable = recordParam.isDeployment && get<4>(valids).count(pc);
