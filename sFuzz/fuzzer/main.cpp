...
static int DEFAULT_ORDER = FIRST;  // default transaction sequence
...
int main(int argc, char* argv[])
{
    ...
    desc.add_options()("help,h", "help message")("contracts,c", po::value(&contractsFolder),
        "contract's folder path")("generate,g", "generate fuzzMe script")(
        "assets,a", po::value(&assetsFolder), "asset's folder path")("file,f", po::value(&jsonFile),
        "fuzz a contract")("name,n", po::value(&contractName), "contract name")(
        "source,s", po::value(&sourceFile), "contract source code path")(
        "mode,m", po::value(&mode), "choose mode: 0 - default AFL mode , 1 - use branch mask")(
        ...
        "prefuzz,p", "get path prefix of each branch")("testcases,t", po::value(&case_num),
        "expected test cases number")("order,o", po::value(&order),
        "switch sequence order: 0 - default | 1 - Swap two functions that have the same priority");
    ...
    if (vm.count("generate"))
    {
        ...
        fuzzMe << fuzzJsonFiles(contractsFolder, assetsFolder, duration, case_num, mode, reporter,
            order, attackerName, vm.count("prefuzz"));
        fuzzMe.close();
        showGenerate();
        return 0;
    }
    ...
    if (vm.count("file") && vm.count("name") && vm.count("source"))
    {
        ...
        fuzzParam.order = (FuzzOrder)order;
