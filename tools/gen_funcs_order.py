...
Date: Created on 3:12 PM 2022-10-20  
"""
...
# initialize the function list -- fdList & set the function order of each function to 0
def init_fdList(abis):
    for node in abis:
        ...
        if node["type"] == "fallback":
            ...
            funcs_order1["fallback," + node["stateMutability"] + ":" + param_len] = 0
            funcs_isRepeat["fallback," + node["stateMutability"] + ":" + param_len] = 0
            funcs_Equal["fallback," + node["stateMutability"] + ":" + param_len] = ''
        if node["type"] == "function" and not node["constant"]:
            fdList.append(node["name"])
            funcs_order[node["name"] + ":" + param_len] = 0
            funcs_order1[node["name"] + ":" + param_len] = 0
            funcs_isRepeat[node["name"] + ":" + param_len] = 0
            funcs_Equal[node["name"] + ":" + param_len] = ''


# Get global variables and type mapping, initialize param_list
...
def get_funcsOrder():
    if funcs_dataDependency:
        ...
        if has_constructor:
            ...
            funcs_order1[constructor_name] = i
        for funcs in funcs_Equal.items():
            if funcs[1] != '':
                # print("1111111")
                print(funcs_order1[funcs[1]])
                index = funcs_order1[funcs[1]]
                funcs_order1[funcs[1]] = funcs_order1[funcs[0]]
                funcs_order1[funcs[0]] = index
        return i
